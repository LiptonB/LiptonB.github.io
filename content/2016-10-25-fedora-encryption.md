...
Check out the structure of this btrfs filesystem
...
Create read-only snapshots of the subvolumes
    ...
    [liveuser@localhost ~]$ sudo btrfs subvolume snapshot -r /mnt/fedora/home /mnt/fedora/homeBACKUP
    Create a readonly snapshot of '/mnt/fedora/home' in '/mnt/fedora/homeBACKUP'

Back up the snapshots to the backup volume

    [liveuser@localhost ~]$ sudo btrfs send /mnt/fedora/rootBACKUP/ | sudo btrfs receive /mnt/backup/
    [liveuser@localhost ~]$ sudo btrfs send /mnt/fedora/homeBACKUP/ | sudo btrfs receive /mnt/backup/
...
Format/encrypt the partition as a LUKS volume. You will be asked to set a passphrase.
    ...
    [liveuser@localhost ~]$ sudo cryptsetup luksFormat /dev/sda4

Unlock the volume. The decrypted partition will be accessible as `/dev/mapper/fedora`.

    [liveuser@localhost ~]$ sudo cryptsetup luksOpen /dev/sda4 fedora

Create a btrfs filesystem on the encrypted partition and copy the data back to it

    [liveuser@localhost ~]$ sudo mkfs.btrfs /dev/mapper/fedora
    [liveuser@localhost ~]$ sudo mount /dev/mapper/fedora /mnt/fedora
    [liveuser@localhost ~]$ sudo btrfs send /mnt/backup/homeBACKUP/ | sudo btrfs receive /mnt/fedora/
    [liveuser@localhost ~]$ sudo btrfs send /mnt/backup/rootBACKUP/ | sudo btrfs receive /mnt/fedora/
    [liveuser@localhost ~]$ sudo mv /mnt/fedora/rootBACKUP/ /mnt/fedora/root
    [liveuser@localhost ~]$ sudo mv /mnt/fedora/homeBACKUP/ /mnt/fedora/home
    [liveuser@localhost ~]$ sudo btrfs subvolume list /mnt/fedora
    ID 257 gen 11 top level 5 path home
    ID 258 gen 24 top level 5 path root

# Fix up references to the old filesystem

    [liveuser@localhost ~]$ sudo blkid
    ...
    [liveuser@localhost ~]$ cd /mnt/fedora/root
    [liveuser@localhost root]$ sudo btrfs property set -ts /mnt/fedora/root ro false
    [liveuser@localhost root]$ sudo btrfs property set -ts /mnt/fedora/home ro false
    [liveuser@localhost root]$ sudo vi etc/fstab
    # Replace UUID in fstab with /dev/mapper/fedora UUID
    [liveuser@localhost root]$ sudo vi etc/crypttab 
    [liveuser@localhost root]$ sudo cat etc/crypttab
    luks-8d0f7036-74e7-4958-aaaf-36645dc30065    UUID=8d0f7036-74e7-4958-aaaf-36645dc30065    none


Interlude: boot process for EFI, LUKS, and btrfs

    :::bash
    [liveuser@localhost root]$ sudo vi /mnt/efi/EFI/fedora/grub.cfg 
...
Boot livecd again
Find the packages for the latest kernel installed on your system on koji: https://koji.fedoraproject.org/koji/buildinfo?buildID=807875
Download kernel, kernel-core, and kernel-modules packages
